...
**Void clustering during WWW — resolved 2026-05-11 by replacing the BM seeder with a crystalline seed + WWW burn-in.** The historical mechanism is preserved below for context, but the BM seeder (`bm_initial_network`, `_build_trivalent_proximity_graph`, `poisson_disk_pbc`, `random_3regular_graph`) has been removed from the module. The replacement is `crystal_seed_network` + `topology_burn_in`, which gives a uniform low-energy starting point (4³-voxel std ~1.1 at N=1000 after initial relax, vs reference 2.79) with no long-bond stragglers that could drag vertices across the cell. See `claude_context/algorithm.md` §1.5 for the Hemmann/Saba 2026 derivation.
